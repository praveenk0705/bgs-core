...
public class BackgroundServicePluginLogic {
	...
	public ExecuteResult execute(String action, JSONArray data, IUpdateListener listener, Object[] listenerExtras) {
		...
		Log.i(TAG, "Start of Execute");
		try {
			Log.i(TAG, "Withing try block");
			if ((data != null) &&
					(!data.isNull(0)) &&
				...
				Log.i(TAG, "Finding servicename " + serviceName);
				
				ServiceDetails service = null;

				Log.i(TAG, "Services contain " + this.mServices.size() + " records");
				
				if (this.mServices.containsKey(serviceName)) {
					Log.i(TAG, "Found existing Service Details");
					service = this.mServices.get(serviceName);
				} else {
					Log.i(TAG, "Creating new Service Details");
				...
				if (result == null) {
					Log.i(TAG, "Check if the service is running?");
				...
				Log.i(TAG, "ERROR - no servicename");
			}
		} catch (Exception ex) {
			result = new ExecuteResult(ExecuteStatus.ERROR);
	...
	public void onDestroy() {
		...
		try {
			Log.i(TAG, "Checking for services");
			if (this.mServices != null && 
				...
				while( keys.hasMoreElements() ) {
					...
					Log.i(TAG, "Calling service.close()");
					service.close();
				}
			}
		} catch (Throwable t) {
			...
			Log.i(TAG, "Error has occurred while trying to close services", t);
		}
		
		
		this.mServices = null;
		Log.i(TAG, "On Destroy Finish");
	...
	protected class ServiceDetails {
		...
		public ExecuteResult stopService()
		{
			...
			Log.i("ServiceDetails", "stopService called");

			try {
				
				Log.i("ServiceDetails", "Unbinding Service");
				this.mContext.unbindService(serviceConnection);
				
				Log.i("ServiceDetails", "Stopping service");
				if (this.mContext.stopService(this.mService))
				{
					Log.i("ServiceDetails", "Service stopped");
				} else {
					Log.i("ServiceDetails", "Service not stopped");
				}
				result = new ExecuteResult(ExecuteStatus.OK, createJSONResult(true, ERROR_NONE_CODE, ERROR_NONE_MSG));
			} catch (Exception ex) {
				Log.i(LOCALTAG, "stopService failed", ex);
				result = new ExecuteResult(ExecuteStatus.ERROR, createJSONResult(false, ERROR_EXCEPTION_CODE, ex.getMessage()));
			}
		...
		public ExecuteResult enableTimer(JSONArray data)
		{
			...
			try {
				...
				Log.i(LOCALTAG, "enableTimer failed", ex);
				result = new ExecuteResult(ExecuteStatus.ERROR, createJSONResult(false, ERROR_EXCEPTION_CODE, ex.getMessage()));
			}
		...
		public ExecuteResult disableTimer()
		{
			...
			try {
				...
				Log.i(LOCALTAG, "disableTimer failed", ex);
				result = new ExecuteResult(ExecuteStatus.ERROR, createJSONResult(false, ERROR_EXCEPTION_CODE, ex.getMessage()));
			}
		...
		public ExecuteResult registerForBootStart()
		{
			...
			try {
				...
				Log.i(LOCALTAG, "registerForBootStart failed", ex);
				result = new ExecuteResult(ExecuteStatus.ERROR, createJSONResult(false, ERROR_EXCEPTION_CODE, ex.getMessage()));
			}
		...
		public ExecuteResult deregisterForBootStart()
		{
			...
			try {
				...
				Log.i(LOCALTAG, "deregisterForBootStart failed", ex);
				result = new ExecuteResult(ExecuteStatus.ERROR, createJSONResult(false, ERROR_EXCEPTION_CODE, ex.getMessage()));
			}
			...
			try {
				if (this.isServiceRunning()) {
					...
					try {
						...
						Log.i(LOCALTAG, "Processing config JSON from background service failed", e);
				...
				Log.i(LOCALTAG, "setConfiguration failed", ex);
				result = new ExecuteResult(ExecuteStatus.ERROR, createJSONResult(false, ERROR_EXCEPTION_CODE, ex.getMessage()));
			}
		...
		public ExecuteResult runOnce()
		{
			...
			try {
				...
				Log.i(LOCALTAG, "runOnce failed", ex);
				result = new ExecuteResult(ExecuteStatus.ERROR, createJSONResult(false, ERROR_EXCEPTION_CODE, ex.getMessage()));
			}
		...
		public ExecuteResult registerForUpdates(IUpdateListener listener, Object[] listenerExtras)
		{
			...
			try {
				...
				Log.i(LOCALTAG, "regsiterForUpdates failed", ex);
				result = new ExecuteResult(ExecuteStatus.ERROR, createJSONResult(false, ERROR_EXCEPTION_CODE, ex.getMessage()));
			}
		...
		public ExecuteResult deregisterForUpdates()
		{
			...
			try {
				...
				Log.i(LOCALTAG, "deregsiterForUpdates failed", ex);
				result = new ExecuteResult(ExecuteStatus.ERROR, createJSONResult(false, ERROR_EXCEPTION_CODE, ex.getMessage()));
			}
		...
		public void close()
		{
			Log.i("ServiceDetails", "Close called");
			try {
				// Remove the lister to this publisher
				this.deregisterListener();
				
				Log.i("ServiceDetails", "Removing ServiceListener");
				mApi.removeListener(serviceListener);
				Log.i("ServiceDetails", "Removing ServiceConnection");
				this.mContext.unbindService(serviceConnection);
			} catch (Exception ex) {
				// catch any issues, typical for destroy routines
				// even if we failed to destroy something, we need to continue destroying
				Log.i(LOCALTAG, "close failed", ex);
				Log.i(LOCALTAG, "Ignoring exception - will continue");
			}
			Log.i("ServiceDetails", "Close finished");
		}
		...
		private boolean deregisterListener() {
			...
			if (this.isRegisteredForUpdates()) {
				Log.i("ServiceDetails", "Listener deregistering");
				try {
					Log.i("ServiceDetails", "Listener closing");
					...
					Log.i("ServiceDetails", "Listener closed");
				} catch (Exception ex) {
					Log.i("ServiceDetails", "Error occurred while closing the listener", ex);
				}
				
				this.mListener = null;
				this.mListenerExtras = null;
				Log.i("ServiceDetails", "Listener deregistered");
		...
		private boolean bindToService() {
			...
			Log.i(LOCALTAG, "Starting bindToService");
			...
			try {
				...
				Log.i(LOCALTAG, "bindToService failed", ex);
			}

			Log.i(LOCALTAG, "Finished bindToService");
		...
		private ServiceConnection serviceConnection = new ServiceConnection() {
			...
			public void onServiceConnected(ComponentName name, IBinder service) {
				...
				try {
					...
					Log.i(LOCALTAG, "addListener failed", e);
				}
		...
		private void handleLatestResult() {
			Log.i("ServiceDetails", "Latest results received");
			
			if (this.isRegisteredForUpdates()) {
				Log.i("ServiceDetails", "Calling listener");
				...
				try {
					...
					Log.i("ServiceDetails", "Listener finished");
				} catch (Exception ex) {
					Log.i("ServiceDetails", "Listener failed", ex);
					Log.i("ServiceDetails", "Disabling listener");
					this.mListener = null;
					this.mListenerExtras = null;
				}
			} else {
				Log.i("ServiceDetails", "No action performed");
			}
		}
		...
		private JSONObject createJSONResult(Boolean success, int errorCode, String errorMessage) {
			...
			try {
				...
				Log.i(LOCALTAG, "Adding basic info to JSONObject failed", e);
			}

			if (this.mServiceConnected != null && this.mServiceConnected && this.isServiceRunning()) {
				try { result.put("ServiceRunning", true); } catch (Exception ex) {Log.i(LOCALTAG, "Adding ServiceRunning to JSONObject failed", ex);};
				try { result.put("TimerEnabled", isTimerEnabled()); } catch (Exception ex) {Log.i(LOCALTAG, "Adding TimerEnabled to JSONObject failed", ex);};
				try { result.put("Configuration", getConfiguration()); } catch (Exception ex) {Log.i(LOCALTAG, "Adding Configuration to JSONObject failed", ex);};
				try { result.put("LatestResult", getLatestResult()); } catch (Exception ex) {Log.i(LOCALTAG, "Adding LatestResult to JSONObject failed", ex);};
				try { result.put("TimerMilliseconds", getTimerMilliseconds()); } catch (Exception ex) {Log.i(LOCALTAG, "Adding TimerMilliseconds to JSONObject failed", ex);};
			} else {
				try { result.put("ServiceRunning", false); } catch (Exception ex) {Log.i(LOCALTAG, "Adding ServiceRunning to JSONObject failed", ex);};
				try { result.put("TimerEnabled", null); } catch (Exception ex) {Log.i(LOCALTAG, "Adding TimerEnabled to JSONObject failed", ex);};
				try { result.put("Configuration", null); } catch (Exception ex) {Log.i(LOCALTAG, "Adding Configuration to JSONObject failed", ex);};
				try { result.put("LatestResult", null); } catch (Exception ex) {Log.i(LOCALTAG, "Adding LatestResult to JSONObject failed", ex);};
				try { result.put("TimerMilliseconds", null); } catch (Exception ex) {Log.i(LOCALTAG, "Adding TimerMilliseconds to JSONObject failed", ex);};
			}

			try { result.put("RegisteredForBootStart", isRegisteredForBootStart()); } catch (Exception ex) {Log.i(LOCALTAG, "Adding RegisteredForBootStart to JSONObject failed", ex);};
			try { result.put("RegisteredForUpdates", isRegisteredForUpdates()); } catch (Exception ex) {Log.i(LOCALTAG, "Adding RegisteredForUpdates to JSONObject failed", ex);};
		...
		private boolean isServiceRunning()
		{
			...
			try {
				...
				Log.i(LOCALTAG, "isServiceRunning failed", ex);
			}
		...
		private Boolean isTimerEnabled()
		{
			...
			try {
				...
				Log.i(LOCALTAG, "isTimerEnabled failed", ex);
			}
		...
		private Boolean isRegisteredForBootStart()
		{
			...
			try {
				...
				Log.i(LOCALTAG, "isRegisteredForBootStart failed", ex);
			}
		...
		private JSONObject getConfiguration()
		{
			...
			try {
				...
				Log.i(LOCALTAG, "getConfiguration failed", ex);
			}
		...
		private JSONObject getLatestResult()
		{
			...
			try {
				...
				Log.i(LOCALTAG, "getLatestResult failed", ex);
			}
		...
		private int getTimerMilliseconds()
		{
			...
			try {
				...
				Log.i(LOCALTAG, "getTimerMilliseconds failed", ex);
			}
